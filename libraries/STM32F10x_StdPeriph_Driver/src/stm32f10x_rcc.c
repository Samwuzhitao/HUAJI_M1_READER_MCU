...
void RCC_DeInit(void)
{
  ...
  /* Reset SW(00:HSI为AHB), HPRE(0000:AHB不分频), PPRE1(000:APB1不分频), PPRE2(000:APB不分频), 
	 ADCPRE(00:ADC 2分频) and MCO(000:没有时钟输出) bits */
#ifndef STM32F10X_CL
  ...
  /* Reset HSEBYP bit */
  /* HSE晶振没有旁路(只有OSC_IN脚输入晶振) */
  RCC->CR &= (uint32_t)0xFFFBFFFF;

  /* Reset PLLSRC, PLLXTPRE, PLLMUL and USBPRE/OTGFSPRE bits */
  /* Reset PLLSRC(0:PLL时钟源为HSI/2), PLLXTPRE(HSE不分频或者/2作为PLL时钟源), 
     PLLMUL(0000~1111:2倍频~16倍频) and USBPRE/OTGFSPRE(PLL时钟/1.5分频或者不分频作为USB时钟) bits */
...
#else
  /* Disable all interrupts and clear pending bits  */
  /* 向bit16~bit20写1,清除各时钟源就绪中断标识位！ 关闭所有中断  */
  RCC->CIR = 0x009F0000;
#endif /* STM32F10X_CL */
...
void RCC_HSEConfig(uint32_t RCC_HSE)
{
  ...
  /* 配置HSE之前要Reset HSEON and HSEBYP ------------------*/
